...
export default class AdminPanel extends Component {
	...
	getMedicineTypes = () => {
		...
		this.repository.get("lookups", `filter=${lookupFilter}&sort=${sortString}&isDeleted=${true}`).then((res) => {
			let medicineTypes =
				res &&
				res.data &&
	...
	getLookups = () => {
		...
		this.repository.get("lookups", `take=${rows}&skip=${first}&filter=${filter}&sort=${sortString}&includeProperties=${includeProperties}`).then((res) => {
			this.setState(
				{
					first: first,
					rows: rows,
	...
	deleteRow = () => {
		...
		this.repository.delete("lookups", `${selectedLookup.id}?isDeleted=${flag}`).then((res) => {
			this.setState({
	...
	render() {
		...
		const { lookups, totalRecords, rows, first, loading, multiSortMeta, filters, deleteDialog, editDialog, isArchive, lookupType, selectedType, medicineTypeOptions, selectedLookup, startNo, endNo } = this.state;
					...
					<div className="col-md-8">
						<div className="card">
							<div className="card-body">
								...
								<DataTable value={lookups} loading={loading} responsive={true} emptyMessage="No records found" ref={(el) => (this.dt = el)} onSort={this.onSort} sortMode="multiple" multiSortMeta={multiSortMeta} selection={selectedLookup} onSelectionChange={(e) => this.setState({ selectedLookup: e.value })} filters={filters} onFilter={this.onFilter} selectionMode="single" lazy={true} paginatorRight={paginatorRight} paginator={true} rowsPerPageOptions={[10, 30, 45]} rows={rows} totalRecords={totalRecords} first={first} onPage={this.onPageChange}>
									<Column field="name" header={lookupType.label} sortable={true} filter={true} filterMatchMode="contains" />
									{lookupType === lookupTypeEnum.MEDICINENAME && <Column field="parent.name" header="Type" filter={true} filterElement={typeFilter} filterMatchMode="eq" style={{ width: "130px" }} />}
					...
					{editDialog && (
						<form onSubmit={this.handleSubmit} onReset={this.handleReset}>
							<div>
								...
								{lookupType === lookupTypeEnum.MEDICINENAME && <InputField name="parentId" title="Parent Type" options={medicineTypeOptions} value={selectedLookup.parentId || null} onChange={this.handleChange} {...this.state} controlType="dropdown" />}
							</div>

...
export default class Statistics extends Component {
  ...
  render() {
    const { opdStatistics, ipdStatistics, opdActiveTab, ipdActiveTab } =
      this.state;
            ...
            >
              {opdStatistics &&
                Object.keys(opdStatistics)
                  .reverse()
                  .map((year) => {
                    totalOpdPatient = opdStatistics[year].reduce(
                      (total, item) => total + item.totalPatient,
                      0
                    );
                    totalOpdCollection = opdStatistics[year].reduce(
                      (total, item) => total + item.totalCollection,
                      0
                    );
                    totalIpdCollection = ipdStatistics[year].reduce(
                      (total, item) => total + item.totalCollection,
                      0
                    );
                    return (
                      <TabPanel header={year} key={year}>
                        ...
                        <h5>
                          ...
                          <span className="badge badge-secondary ml-2">
                            {totalOpdCollection} + {totalIpdCollection} ={" "}
                            {this.helper.formatCurrency(
                              totalOpdCollection + totalIpdCollection
                            )}
                          </span>
            ...
            >
              {ipdStatistics &&
                Object.keys(ipdStatistics)
                  .reverse()
                  .map((year) => {
                    totalIpdPatient = ipdStatistics[year].reduce(
                      (total, item) => total + item.totalPatient,
                      0
                    );
                    totalIpdCollection = ipdStatistics[year].reduce(

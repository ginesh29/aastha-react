...
export default class IpdInvoice extends Component
{
    ...
    getCharges = () =>
    {
        this.repository.get("lookups", `filter=type-eq-{${ lookupTypeEnum.CHARGENAME.value }} and isDeleted-neq-${ true }`)
            .then(res =>
            {
    ...
    render()
    {
        ...
        chargeNames && chargeNames.map(item =>
        {
            ...
            InvoiceData[chargeName] = { chargeName: item.name, rate: rate, days: days, amount: amount }
        })
        ...
        return (
            ...
            <div>
                ...
                <table className="table table-borderless invoice-detail">
                    <tbody>
                        <tr>
                            <td><b>Name :</b> {InvoiceData.patient.fullname}</td>
                            <td width="200px"><b>Date :</b> {InvoiceData.formatedDischargeDate}</td>
                        </tr>
                        <tr>
                            <td><b>Invoice No. :</b> {InvoiceData.uniqueId}</td>
                            <td><b>Address :</b> {InvoiceData.address}</td>
                        </tr>
                        <tr>
                            <td><b>Indoor No. :</b> {InvoiceData.invoiceNo}</td>
                            <td><b>Room Type :</b> {InvoiceData.roomTypeName}</td>
                        </tr>
                    </tbody>
                </table>
                <div className="row">
                        ...
                        <table className="table table-bordered invoice-table">
                            ...
                            <tfoot className="invoice-footer">
                                <tr>
                                    ...
                                    <td className="text-right"><strong> {this.helper.formatCurrency(totalAmount)}</strong></td>
                                </tr>
                                <tr>
                                    <td colSpan="4">Discount</td>
                                    <td className="text-right"><strong> {this.helper.formatCurrency(InvoiceData.discount)}</strong></td>
                                </tr>
                                <tr>
                                    <td colSpan="4">Net Payable Amount :<span className="text-capitalize"> {`${ amountInWord } Only`}</span></td>
                                    <td className="text-right"><strong> {this.helper.formatCurrency(payableAmount)}</strong></td>
                                </tr>
                            </tfoot>
                        </table>

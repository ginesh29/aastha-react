...
export default class IpdForm extends React.Component {
	...
	handleSubmit = (e) => {
		const { id, uniqueId, departmentType, roomType, patient, addmissionDate, dischargeDate, deliveryDate, deliveryTime, babyGender, babyWeight, typesOfDelivery, operationDiagnosis, typesOfOperation, generalDiagnosis, operationDate, deliveryDiagnosis, discount, deliveryId, operationId, ipdLookups } = this.state.formFields;
		...
		if (this.handleValidation()) {
			...
			const ipd = {
				...
				deliveryDetail: departmentType === departmentTypeEnum.DELIVERY.value ? deliveryDetail : null,
				operationDetail: departmentType === departmentTypeEnum.OPERATION.value ? operationDetail : null,
				ipdLookups: ipdLookupArray,
				charges: charges,
				discount: discount,
			};
			this.repository.post(`${controller}?includeProperties=${includeProperties ? includeProperties : ""}`, ipd).then((res) => {
				if (res && !res.errors) {
	...
	handleValidation = (e) => {
		...
		if (!departmentType) {
			...
			if (departmentType && departmentType === departmentTypeEnum.DELIVERY.value) {
				if (!deliveryDate) {
					isValid = false;
			...
			if (departmentType && departmentType === departmentTypeEnum.OPERATION.value) {
				if (!operationDate) {
					isValid = false;
			...
			if (departmentType && departmentType === departmentTypeEnum.GENERAL.value) {
				if (!generalDiagnosis) {
					isValid = false;
	...
	bindLookups = (e) => {
		this.repository.get("lookups", `filter=type-neq-{0} and isDeleted-neq-${true}`).then((res) => {
			...
			if (res) {
				let typesofDeliveryOptions = lookups.filter((l) => l.type === lookupTypeEnum.DELIVERYTYPE.code);
				let deliveryDiganosisOptions = lookups.filter((l) => l.type === lookupTypeEnum.DELIVERYDIAGNOSIS.code);
				let operationDiagnosisOptions = lookups.filter((l) => l.type === lookupTypeEnum.OPERATIONDIAGNOSIS.code);
				let typesofOprationOptions = lookups.filter((l) => l.type === lookupTypeEnum.OPERATIONTYPE.code);
				let generalDiagnosisOptions = lookups.filter((l) => l.type === lookupTypeEnum.GENERALDIAGNOSIS.code);
		...
		if (selectedIpd && selectedIpd.id) {
			...
			if (selectedIpd.ipdLookups) {
				...
				selectedIpd.ipdLookups.map((item) => {
					...
					if (item.lookup.type === lookupTypeEnum.DELIVERYDIAGNOSIS.code) deliveryDiagnosis = obj;
					else if (item.lookup.type === lookupTypeEnum.DELIVERYTYPE.code) typesOfDelivery = typesOfDelivery.concat(obj);
					else if (item.lookup.type === lookupTypeEnum.OPERATIONDIAGNOSIS.code) operationDiagnosis = operationDiagnosis.concat(obj);
					else if (item.lookup.type === lookupTypeEnum.OPERATIONTYPE.code) typesOfOperation = typesOfOperation.concat(obj);
					else if (item.lookup.type === lookupTypeEnum.GENERALDIAGNOSIS.code) generalDiagnosis = generalDiagnosis.concat(obj);
				});
				...
				<form onSubmit={this.handleSubmit} onReset={this.handleReset}>
					<div className="row">
						<div className="col-md-4">
							<InputField name="uniqueId" title="Indoor No." value={uniqueId || ""} onChange={this.handleChange} {...this.state} keyfilter="pint" className={id ? `p-readonly` : ""} />
						</div>
						<div className="col-md-4">
							<InputField
								name="patient"
						...
						<div className="col-md-4">
							<InputField name="roomType" title="Room Type" value={roomType || null} onChange={this.handleChange} {...this.state} controlType="dropdown" options={roomTypeOptions} />
						</div>
					</div>
					<div className="row">
						<div className="col-md-4">
							<InputField name="departmentType" title="Department Type" value={departmentType || null} onChange={this.handleChange} {...this.state} controlType="dropdown" options={departmentTypeOptions} className={id ? `p-readonly` : ""} />
						</div>
						<div className="col-md-4">
					...
					{departmentType && departmentType === departmentTypeEnum.DELIVERY.value && (
						<>
							<div className="row">
								<div className="col-md-4">
								...
								<div className="col-md-4">
									<InputField name="typesOfDelivery" title="Types Of Delivery" value={typesOfDelivery || null} onChange={this.handleChange} {...this.state} controlType="multiselect" options={typesofDeliveryOptions} filter={true} />
								</div>
							</div>
							<div className="row">
								<div className="col-md-4">
									<InputField name="deliveryDiagnosis" title="Delivery Diagnosis" value={deliveryDiagnosis || null} onChange={this.handleChange} {...this.state} controlType="dropdown" options={deliveryDiganosisOptions} />
								</div>
								<div className="col-md-4">
									<InputField name="babyGender" title="Baby Gender" value={babyGender || null} onChange={this.handleChange} {...this.state} controlType="dropdown" options={genderOptions} />
								</div>
								<div className="col-md-4">
					...
					{departmentType && departmentType === departmentTypeEnum.OPERATION.value && (
						<div className="row">
							<div className="col-md-4">
							...
							<div className="col-md-4">
								<InputField name="operationDiagnosis" title="Operation Diagnosis" value={operationDiagnosis || null} onChange={this.handleChange} {...this.state} controlType="multiselect" options={operationDiagnosisOptions} filter={true} />
							</div>
							<div className="col-md-4">
								<InputField name="typesOfOperation" title="Types Of Operation" value={typesOfOperation || null} onChange={this.handleChange} {...this.state} controlType="multiselect" options={typesofOprationOptions} filter={true} />
							</div>
						</div>
					)}
					{departmentType && departmentType === departmentTypeEnum.GENERAL.value && (
						<div className="row">
							<div className="col-md-4">
								<InputField name="generalDiagnosis" title="General Diagnosis" value={generalDiagnosis || null} onChange={this.handleChange} {...this.state} controlType="multiselect" options={generalDiagnosisOptions} filter={true} />
							</div>
						</div>
					)}
						...
						<tbody>
							{chargeNames &&
								chargeNames.map((item, index) => {
									...
									let amount = "";
									if (chargeFormFields) {
										const chargeObj = chargeFormFields.filter((c) => c.lookupId === item.value)[0];
									...
									return (
										<tr key={index}>
											...
											<td>{item.label}</td>
											<td>
												<InputText type="text" value={rate} className="p-inputtext-sm" keyfilter="pint" name={`rate-${item.value}`} onChange={this.handleChargeChange} />
											</td>
											<td>
												<InputText type="text" value={days} className="p-inputtext-sm" keyfilter="pint" name={`days-${item.value}`} onChange={this.handleChargeChange} />
											</td>
											<td>{amount}</td>
										</tr>
									);
							...
							<tr>
								...
								<td colSpan="3">
									<div className="d-flex flex-row bd-highlight">
										<div className="mr-2">
											<InputText type="text" className="p-inputtext-sm" keyfilter="pint" value={grandTotal || ""} readOnly />
										</div>
										<div className="mr-2 mt-1">
											<i className="fa fa-minus"></i>
										</div>
										<div>
											<InputText name="discount" className="p-inputtext-sm" type="text" keyfilter="pint" value={discount || ""} onChange={this.handleChargeChange} />
										</div>
									</div>
								</td>
							</tr>

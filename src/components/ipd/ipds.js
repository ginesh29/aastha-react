...
export default class Ipds extends Component {
    ...
    saveIpd = (updatedIpd, id) => {
        ...
        let ipdData = [...ipds];
        updatedIpd.patient = { label: updatedIpd.patient.fullname, value: updatedIpd.patientId, fullname: updatedIpd.patient.fullname }
    ...
    render() {
        const { ipds, totalRecords, rows, first, loading, multiSortMeta, filters, deleteDialog, selectedIpd,
            isArchive, selectedAddmissionDate, selectedDischargeDate, selectedIpdType, editDialog, includeProperties, invoiceDialog } = this.state;
            ...
            <>
                <div className="card">
                    <div className="card-body">
                        ...
                        <DataTable value={ipds} loading={loading} responsive={true} emptyMessage="No records found" ref={(el) => this.dt = el}
                ...
                <Dialog header="Edit Ipd" visible={editDialog} onHide={() => { this.setState({ editDialog: false }) }} className="p-scroll-dialog">
                    {
                        editDialog &&
                        <IpdForm selectedIpd={selectedIpd} hideEditDialog={() => this.setState({ editDialog: false })} saveIpd={this.saveIpd} includeProperties={includeProperties} />
                    }
                </Dialog>

...
export default class Opds extends Component {
	...
	render() {
		const { opds, totalRecords, rows, first, loading, multiSortMeta, filters, deleteDialog, isArchive, selectedOpd, editDialog, includeProperties, selectedCaseType, selectedDate, invoiceDialog, startNo, endNo } = this.state;
		...
		return (
			<>
				...
				<Dialog header="Edit Opd" visible={editDialog} onHide={() => this.setState({ editDialog: false })} dismissableMask={true}>
				...
				<Dialog header="Opd Invoice" visible={invoiceDialog} onHide={() => this.setState({ invoiceDialog: false })} className="p-scroll-dialog" style={{ width: "550px" }} dismissableMask={true}>
					{invoiceDialog && (
						<>
							...
							<div className="modal-footer">
								<button type="button" className="btn bt-sm btn-secondary" onClick={() => jquery("#print-div").print()}>

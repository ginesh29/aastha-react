...
export default class Prescription extends React.Component {
	...
	bindLookups = (e) => {
		this.repository.get("lookups", `filter=(type-eq-{${lookupTypeEnum.MEDICINETYPE.code}} or type-eq-{${lookupTypeEnum.ADVICE.code}}) and isDeleted-neq-${true}`).then((res) => {
			let lookups =
				res &&
			...
			if (res) {
				let medicineTypeOptions = lookups.filter((l) => l.type === lookupTypeEnum.MEDICINETYPE.code);
				let adviceOptions = lookups.filter((l) => l.type === lookupTypeEnum.ADVICE.code);
	...
	bindMedicineName = (medicineType) => {
		...
		this.repository.get("lookups", `filter=parentId-eq-{${medicineType.code}}`).then((res) => {
			let medicineNames =
				res &&
				res.data.map(function (item) {
													...
													<div className="d-flex" key={i + 1}>
														...
														{item.value <= 4 && followup === item.value && (
															<>
																<div className="mr-2">
																	<InputText
																		className={`p-inputtext-sm ${validationErrors.followupDate ? "error" : ""}`}
								...
								<div className="col-md-6">
									...
									<div className="row gujarati-text mt-3">
										{medicineInstructionOptions.map((item, i) => {
											return (
												<div className={`instruction-check col-md-${item.label.length < 15 ? "3" : item.label.length <= 40 ? "6" : "12"} mb-1`} key={i + 1}>

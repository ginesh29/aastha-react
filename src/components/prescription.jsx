...
export default class Prescription extends React.Component {
  ...
  bindLookups = e => {
    this.repository
      .get(
        "lookups",
        `filter=(type-eq-{${lookupTypeEnum.MEDICINETYPE.value}} or type-eq-{${
          lookupTypeEnum.ADVICE.value
      ...
      .then(res => {
        ...
        if (res) {
          let medicineTypeOptions = lookups.filter(
            l => l.type === lookupTypeEnum.MEDICINETYPE.value
          );
          let adviceOptions = lookups.filter(
            l => l.type === lookupTypeEnum.ADVICE.value
          );
  ...
  bindMedicineName = medicineType => {
    ...
    this.repository
      .get("lookups", `filter=parentId-eq-{${medicineType}}`)
      .then(res => {
        let medicineNames =
          res &&
          res.data.map(function(item) {
      ...
      <>
        ...
        <div className="float-right">
          <ReactToPrint
            ...
            onAfterPrint={this.saveAppointment}
          />
        </div>
      </>
    );
                    ...
                    <table>
                      <tbody>
                        <tr>
                          ...
                          <td className="gujarati-text align-bottom">
                            {followupInstruction}
                          </td>
                        </tr>
                      </tbody>
                        ...
                        >
                          <Checkbox
                            ...
                            onChange={e =>
                              ...
                              this.handleMedicineChange(e, item.value)
                            }
                          />
                          <label
